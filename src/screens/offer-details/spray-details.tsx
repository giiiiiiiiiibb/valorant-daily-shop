    ...
    <View style={[styles.container, { backgroundColor: palette.background }]}>
      ...
      <ScrollView showsVerticalScrollIndicator={false} style={styles.scrollView} contentContainerStyle={styles.scrollViewContent}>
        {[spray.fullTransparentIcon, spray.fullIcon].map((icon, index) => (
          <View key={index} style={[styles.iconContainer, { backgroundColor: palette.card }]}>
            <Image borderRadius={16} resizeMode="contain" source={{ uri: icon }} style={styles.image} />
          </View>
        ))}
      </ScrollView>
    </View>
...
const styles = StyleSheet.create({
  ...
  displayName: { fontFamily: "Vandchrome" },
  sprayText: { opacity: 0.5, textTransform: "uppercase" },
  scrollView: { flex: 1 },
  scrollViewContent: { gap: 16, justifyContent: "center", alignItems: "center" },
  iconContainer: { borderRadius: 32, padding: 16 },
  image: { width: WIDTH - 64, height: WIDTH - 64 },
});
