...
const CollectionDetailsScreen = ({ route }: CollectionDetailScreenProps) => {
  ...
  const [currentLevelIndex, setCurrentLevelIndex] = useState<number | undefined>(
    currentPlayerLoadoutSkin?.levels.findIndex((level) => level.uuid === currentPlayerLoadoutGun?.SkinLevelID) ?? 0
  );

  const [currentChromaIndex, setCurrentChromaIndex] = useState<number>(
    currentPlayerLoadoutSkin?.chromas.findIndex((chroma) => chroma.uuid === currentPlayerLoadoutGun?.ChromaID) ?? 0
  );
  ...
  const onCardPress = (skinSelected: WeaponSkin) => {
    ...
    if (skinSelected.uuid === currentPlayerLoadoutSkin?.uuid) {
      const chromaIndex = skinSelected.chromas.findIndex((chroma) => chroma.uuid === currentPlayerLoadoutGun?.ChromaID);
      const levelIndex = skinSelected.levels.findIndex((level) => level.uuid === currentPlayerLoadoutGun?.SkinLevelID);
  ...
  const { data: weaponData, isLoading: isLoadingWeapon } = useGetWeaponByIdQuery(currentPlayerLoadoutGun?.ID ?? "");
  ...
  const disabled = useMemo(() => {
    ...
    const isNotOwned = !skins?.Entitlements.some(
      (entitlement) => currentSkin.levels[currentLevelIndex ?? 0]?.uuid === entitlement.ItemID
    );
    const isNotOwnedChroma =
      !skinVariants?.Entitlements.some((entitlement) => currentSkin.chromas[currentChromaIndex]?.uuid === entitlement.ItemID) &&
    ...
    if (
      ...
      currentSkin.levels[currentLevelIndex]?.uuid === currentPlayerLoadoutGun?.SkinLevelID
    ) {
      return true;
    }
  ...
  const isFavorite = useMemo(() => {
    return Object.keys(favoriteSkins?.FavoritedContent ?? {}).some(
      (key) => favoriteSkins?.FavoritedContent[key].ItemID === currentSkin.chromas[currentChromaIndex].uuid
    );
  ...
  const isFavoriteDisabled = useMemo(() => {
    ...
    if (
      !skins?.Entitlements.some((entitlement) => currentSkin.levels.some((level) => level.uuid === entitlement.ItemID))
    ) {
      return true;
    }
  ...
  const [currentImage, setCurrentImage] = useState(
    currentPlayerLoadoutSkin?.chromas.find((chroma) => chroma.uuid === currentPlayerLoadoutGun?.ChromaID)?.fullRender ||
      currentPlayerLoadoutSkin?.chromas[0].fullRender
  );
  ...
  const sortWeaponSkinsByOwned = () => {
    ...
    copy.sort((a, b) => {
      ...
      const aOwned = skins?.Entitlements.some((entitlement) => a.levels.some((level) => level.uuid === entitlement.ItemID));
      const bOwned = skins?.Entitlements.some((entitlement) => b.levels.some((level) => level.uuid === entitlement.ItemID));
  ...
  useEffect(() => {}, [currentPlayerLoadoutGun, favoriteSkins]);
  ...
  const handleChromaPress = useCallback(
    (index: number, fullRender: string) => {
  ...
  return (
    <View style={[styles.container, { backgroundColor: palette.background }]}>
      <View style={styles.header}>
        ...
        <Image resizeMode="contain" source={getContentTierIcon(currentSkin.contentTierUuid)} style={styles.icon} />
      </View>
      ...
      <View style={{ gap: 8, display: "flex", maxWidth: "100%", flexDirection: "row" }}>
        <Button
          style={{ maxWidth: 64 }}
