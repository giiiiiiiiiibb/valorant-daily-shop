...
const CollectionWeaponSkinList: React.FC<Props> = ({
  ...
  const indicatorStyle = useMemo(() => (isDark ? "white" : "black"), [isDark]);

  const renderItem = ({ item }: { item: WeaponSkin }) => {
    const isCurrentSkin = currentSkin.uuid === item.uuid;
    const isLocked =
      item.themeUuid !== "0d7a5bfb-4850-098e-1821-d989bbfd58a8" &&
      ...
      <TouchableRipple
        ...
        style={[
          styles.card,
          {
            ...
            borderColor: isCurrentSkin ? palette.primary : palette.card,
          },
        ]}
      >
        <>
          ...
          />
          {isLocked && (
            <View style={[styles.lockOverlay, { backgroundColor: hexToRgba(palette.primary, 0.2) }]}>
