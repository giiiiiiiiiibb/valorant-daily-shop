...
const CollectionCard = ({ playerLoadoutGun }: Props) => {
  ...
  }, [navigate, weaponSkinData, themeData, playerLoadoutGun, setCurrentPlayerLoadoutGun, setCurrentPlayerLoadoutSkin]);
  ...
  return (
    ...
    >
      <ImageBackground resizeMode="cover" style={styles.imageBackground} source={{ uri: skinData.wallpaper }}>
        <Text variant="titleLarge" numberOfLines={1}>
          {getWeaponByUuid(playerLoadoutGun.ID)}
        </Text>
        <Image
          resizeMode="contain"
          style={styles.skinImage}
          source={{
            uri: skinData.chromas.find((weaponChroma) => weaponChroma.uuid === playerLoadoutGun.ChromaID)?.fullRender,
          }}
        />
      </ImageBackground>
    </TouchableRipple>
