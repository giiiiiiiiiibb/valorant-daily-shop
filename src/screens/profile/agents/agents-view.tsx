...
const AgentsView = () => {
  ...
  const isOwned =
    ownedAgents?.Entitlements.some((agent) => agent.ItemID === agents[agentIndex].uuid) ||
    agents[agentIndex].isBaseContent;
      ...
      <View style={styles.agentInfo}>
        <View style={styles.agentInfoOverlay}>
          ...
          <View style={styles.ownershipStatus}>
            {isOwned ? (
              <>
                <SvgCheck color={palette.text} width={24} height={24} />
                <Text variant="titleMedium">Owned</Text>
              </>
            ) : (
              <>
                <SvgLock color={palette.text} width={24} height={24} />
                <Text variant="titleMedium">Not owned</Text>
              </>
            )}
          </View>
          <View style={[styles.agentRoleContainer, { backgroundColor: palette.card }]}>
            <Image style={styles.roleIcon} source={{ uri: agents[agentIndex].role.displayIcon }} />
            <Text variant="titleMedium">{agents[agentIndex].role.displayName}</Text>
          </View>
        </View>
        <Image style={[styles.agentPortrait, { width: WIDTH }]} source={{ uri: agents[agentIndex].fullPortrait }} />
        <Image style={styles.agentBackground} source={{ uri: agents[agentIndex].background }} />
      </View>
      <View style={[styles.agentListContainer, { backgroundColor: palette.background }]}>
        <FlatList
          horizontal
          data={agents}
