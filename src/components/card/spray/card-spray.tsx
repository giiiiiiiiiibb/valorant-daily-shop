...
const CardSpray = ({ offer }: CardSprayProps) => {
  ...
  const { data: sprayData, error: sprayError, isLoading: isLoadingSpray } = useGetSprayByIdQuery(
  ...
  const renderSprays = useMemo(() => {
    ...
    return [sprayData.data.fullIcon, sprayData.data.fullTransparentIcon, sprayData.data.displayIcon].map(
      (iconUrl, index) => (
        <View key={index} style={styles.sprayImageContainer}>
          <Image borderRadius={8} source={{ uri: iconUrl }} style={styles.sprayImage} resizeMode="contain" height={512} />
        </View>
      )
    );
  }, [sprayData]);
  ...
  return (
    ...
    >
      <ImageBackground style={styles.imageBackground} blurRadius={50} source={{ uri: spray.displayIcon }}>
        <Text variant="titleLarge" style={styles.title} numberOfLines={1}>
          {removeCardType(spray.displayName, "spray")}
        </Text>
        <View style={styles.sprayContainer}>{renderSprays}</View>
